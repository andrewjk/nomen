#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "main.h"

int malloc_count;

// Trait Disposable
typedef struct Disposable
{
} Disposable;
void Disposable_dispose(struct Disposable *self)
{
struct Disposable _self = *self;
Console_write("\nboom!");
}

// Trait Stringable
typedef struct Stringable
{
} Stringable;

// Trait Animal
typedef struct Animal
{
} Animal;
char* Animal_speak(struct Animal *self)
{
struct Animal _self = *self;
return "...";
}

// Struct int
char* int_to_string(long self)
{
int length = snprintf(NULL, 0, "%ld", self);
char* str = malloc(length + 1);
malloc_count++;
snprintf(str, length + 1, "%ld", self);
return str;
}

// Struct uint
char* uint_to_string(unsigned long self)
{
int length = snprintf(NULL, 0, "%ld", self);
char* str = malloc(length + 1);
malloc_count++;
snprintf(str, length + 1, "%ld", self);
return str;
}

// Struct int8
char* int8_to_string(char self)
{
int length = snprintf(NULL, 0, "%d", self);
char* str = malloc(length + 1);
malloc_count++;
snprintf(str, length + 1, "%d", self);
return str;
}

// Struct uint8
char* uint8_to_string(unsigned char self)
{
int length = snprintf(NULL, 0, "%d", self);
char* str = malloc(length + 1);
malloc_count++;
snprintf(str, length + 1, "%d", self);
return str;
}

// Struct float
char* float_to_string(float self)
{
int length = snprintf(NULL, 0, "%f", self);
char* str = malloc(length + 1);
malloc_count++;
snprintf(str, length + 1, "%f", self);
return str;
}

// Struct string
char* string_to_string(char* self)
{
return self;
}

// Struct Console
typedef struct Console
{
void *_vt;
} Console;
Console Console_init()
{
Console c;
return c;
}
void Console_write(char* line)
{
printf("%s", line);
}

// Struct Dog
void *_Dog_Animal_funcs[] = {Dog_speak, get_Dog_name, set_Dog_name, get_Dog_age, set_Dog_age};
void *_Dog_Disposable_funcs[] = {Disposable_dispose};
void *_Dog_traits[] = {&_Dog_Disposable_funcs, NULL, &_Dog_Animal_funcs};
typedef struct Dog
{
void *_vt;
char* name;
long age;
} Dog;
Dog Dog_init()
{
Dog d;
d._vt = &_Dog_traits;
d.name = "Dog";
d.age = 1;
return d;
}
char* Dog_speak(struct Dog *self)
{
struct Dog _self = *self;
return "woof";
}
char* get_Dog_name(struct Dog *self) { return self->name; }
void set_Dog_name(struct Dog *self, char* value) { self->name = value; }
long get_Dog_age(struct Dog *self) { return self->age; }
void set_Dog_age(struct Dog *self, long value) { self->age = value; }

// Struct Cat
void *_Cat_Animal_funcs[] = {Cat_speak, get_Cat_name, set_Cat_name, get_Cat_age, set_Cat_age};
void *_Cat_traits[] = {NULL, NULL, &_Cat_Animal_funcs};
typedef struct Cat
{
void *_vt;
char* name;
long age;
} Cat;
Cat Cat_init()
{
Cat c;
c._vt = &_Cat_traits;
c.name = "Cat";
c.age = 1;
return c;
}
char* Cat_speak(struct Cat *self)
{
struct Cat _self = *self;
return _self.name;
}
char* get_Cat_name(struct Cat *self) { return self->name; }
void set_Cat_name(struct Cat *self, char* value) { self->name = value; }
long get_Cat_age(struct Cat *self) { return self->age; }
void set_Cat_age(struct Cat *self, long value) { self->age = value; }

// Struct Lizard
void *_Lizard_Animal_funcs[] = {Animal_speak, get_Lizard_name, set_Lizard_name, get_Lizard_age, set_Lizard_age};
void *_Lizard_traits[] = {NULL, NULL, &_Lizard_Animal_funcs};
typedef struct Lizard
{
void *_vt;
char* name;
long age;
} Lizard;
Lizard Lizard_init()
{
Lizard l;
l._vt = &_Lizard_traits;
l.name = "Lizard";
l.age = 1;
return l;
}
char* get_Lizard_name(struct Lizard *self) { return self->name; }
void set_Lizard_name(struct Lizard *self, char* value) { self->name = value; }
long get_Lizard_age(struct Lizard *self) { return self->age; }
void set_Lizard_age(struct Lizard *self, long value) { self->age = value; }

// Func main
int main()
{
Console_write("\n");
long i;
for (i = 0; i < 5; i++)
{
Console_write("hello, world! ");
if (i + 1 < 4) {
char* _param_0 = int_to_string(i + 1);
char* _param_1 = _string_interpolate_1("%s is less than 4", _param_0);
Console_write(_param_1);

// Auto-free
free(_param_0);
malloc_count--;
free(_param_1);
malloc_count--;
} else {
char* _param_2 = int_to_string(i + 1);
char* _param_3 = _string_interpolate_1("%s is 4 or more", _param_2);
Console_write(_param_3);

// Auto-free
free(_param_2);
malloc_count--;
free(_param_3);
malloc_count--;
}
Console_write("\n");
}
Console_write("\n");
long y = 0;
while (y < 5) {
Console_write("hi ");
y = y + 1;
}
Console_write("\n\n");
Dog dog = make_dog();
char* _param_4 = dog.name;
Console_write(_param_4);
Console_write(": ");
char* _param_5 = Dog_speak(&dog);
Console_write(_param_5);
Console_write("\n\n");
Dog _animals_1 = Dog_init();
Cat _animals_2 = Cat_init();
Lizard _animals_3 = Lizard_init();
void *animals[3] = {&_animals_1, &_animals_2, &_animals_3};
for (int i = 0; i < 3; i++)
{
void *a = *(animals + i);
char* _param_6 = ((char *(*)(void *))_get_trait_func(a, 2, 0))(a);
Console_write(_param_6);
Console_write("\n");
}
Cat cat = Cat_init();
grow_animal((void *)&cat);
Console_write("cat has age: ");
char* _param_7 = int_to_string(cat.age);
Console_write(_param_7);

// Auto-free
((void *(*)(void *))_get_trait_func((void *)&dog, 0, 0))(&dog);
((void *(*)(void *))_get_trait_func((void *)&_animals_1, 0, 0))(&_animals_1);
free(_param_7);
malloc_count--;

printf("\n\nMalloc balance: %d\n", malloc_count);
}

// Func make_dog
struct Dog make_dog()
{
Dog dog = Dog_init();
return dog;
}

// Func grow_animal
void grow_animal(struct Animal *animal)
{
((void (*)(void *, long))_get_trait_func((void *)animal, 2, 4))(animal, ((long(*)(void *))_get_trait_func((void *)animal, 2, 3))(animal) + 1);
}


void **_get_trait_func(void **obj, int trait_index, int func_index)
{
    void **vt = *obj;
    void **trait = *(vt + trait_index);
    void **func = *(trait + func_index);
    return func;
}  
char *_string_interpolate_1(char *pattern, char *arg1)
{
    int length = snprintf(NULL, 0, pattern, arg1);
    char *str = malloc(length + 1);
    malloc_count++;
    snprintf(str, length + 1, pattern, arg1);
    return str;
}
