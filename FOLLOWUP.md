...
## Regex pattern escapes: `\r`/`\n`/`\t` (any unrecognized escape) silently match a literal letter

The engine (`System::Text::Regex`) recognizes only the shorthands
(`\d \D \w \W \s \S`), backreferences (`\1`-`\9`), and escaped
metacharacters. Every other escape — most notably `\r`, `\n`, `\t`, but
also typos like `\z` — is treated as the LITERAL letter, with no compile
or runtime diagnostic:

```
Regex.test("\r\n|\n", "line1\nline2")   // pattern text is \r, \n → letters r, n
// → false (matches literal "rn"), silently
```

Users coming from PCRE/JS/Python expect `\r`/`\n`/`\t` to mean CR/LF/TAB
and get silent wrong matches. (Real control bytes DO work — spelled via
nomen's own string escapes, `"\r?\n"` in source — which is how the
allmark port works around it; discovered while verifying the first-byte
prefilter fix, where a probe spelled `"\\r?\\n"` and expected newline
matching. POSIX ERE leaves these escapes undefined, so treating them as
literals is standards-legal — just surprising and silent.)

Two-part closure (both cheap, independent):

1. **Engine support**: map `\r` → 13, `\n` → 10, `\t` → 9 in the escaped
   atoms of `match_single_atom`, `char_in_class`, and the first-byte scan
   (`alt_first_bytes` appends the escaped char — it would append 'r'
   today for a `\r` atom; after the change the real CR, keeping the
   prefilter sound). No ERE compatibility concern; matches every other
   engine's behavior.
2. **Compile-time lint**: the Regex entry points (`test`, `match`,
   `count`, `find`, `replace_all`, `captures`, and the `_ci` variants)
   take the pattern as their FIRST argument — when that argument is a
   static string literal, check-time could flag escapes outside the
   known set (`\d\D\w\W\s\S`, `\1`-`\9`, escaped metacharacters, and —
   after (1) — `\r\n\t`) as a probable typo (`\z`), since dynamic
   runtime patterns can't be validated this way.

Related trap worth a SPEC note either way: nomen string `"\r"` is a real
CR byte, and a regex pattern is just a string — so the SAME two
characters can mean CR (nomen escape) or letter-r (engine escape)
depending on the backslash count in the source.
