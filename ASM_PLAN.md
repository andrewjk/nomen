...
### NIR-driven emission tranche 3 (DONE): NIR-level expression emission + `return` takeover

The last structured statement kind at the dispatch point is now emitted
NIR-natively, and expressions got their own seam.

- `emit_stmt_from_nir` gained the `return` arm: `build_return_node` takes the
  lowered `NirExpr` as an optional third argument and drives its value
  emission through the new expression seam (general value path AND the
  array-literal element fallback, whose NIR `call` args are index-aligned
  element exprs). The trailing-newline replicates build_node's
  with_semicolon tail, keeping output byte-identical to the delegated path.
  Everything semantic stays on the AST node inside the builder (nullable-null
  sret flag, join-slot for `return match/switch/if`, override-ctor temp
  routing, string-literal strdup, borrow normalization, `move_T` heap marks,
  sret copy, array ownership marks) — same split as tranches 1–2. The
  `return match` join path is unreachable under a NIR ctx (a match in value
  position lowers to `other` → `unknown_kinds` non-empty → whole-function
  AST fallback), asserted white-box by a test.
- `emit_expr_from_nir(expr, status)` — the EXPRESSION seam: exhaustive,
  never-guarded dispatch over `NirExpr`. Each kind routes to the same builder
  the AST walk picked, via `build_node` so the hoisted-allocation pre-pass is
  preserved; `wrap`+`grouped` recurses through the NIR inner (build_node's
  grouped case is a pure pass-through, so the NIR-side recursion is the same
  descent). A new expression kind is a compile error here until mapped —
  this is where liveness-gated decisions, promoted-register leaves and NEON
  expression lowering attach in later tranches.

Proof: byte-IDENTICAL `.s` versus the emission cursor disabled —
`test/emit_nir.test.ts` extended to 22 tests (returns of every expression
shape: leaf/binary/grouped/cast/call; returns from if/while/for arms; float
d0 returns; string borrow-normalized returns; array-literal return riding
the NIR element facts; white-box `return match` → `match` in unknown_kinds)
AND the full `bench/nomen/*.nm` corpus. Mutation checks (markers in the
`return` dispatch arm and the `leaf` expression arm) confirmed both paths
execute and were reverted. Behavioral test runs the NIR-built binary
(int/float/string returns → `5 0 40 0 9 14 -9 5.000000 0.750000 hi world
bob`). Full suite green (256 files / 2530 tests). Perf: byte-identical
output by design.

### NIR-driven emission tranche 4 (DONE): `declare`/`assign`/`eval` value positions

Every statement kind with a value position now emits NIR-natively at the
dispatch point.

- `emit_stmt_from_nir` gained `declare`, `assign` and `eval` arms. All three
  keep every semantic decision on the AST node inside the builder (type
  routing, ownership/reclamation, aliasing, move marks, swap marshalling) and
  route only the VALUE through the seam:
  - `declare` → `build_declaration_node(node, status, nir_init)` — ~20
    init-emission sites (primitives, strings, struct/class/enum/trait locals,
    array-from-call, view locals, if/match/switch joins, sret paths) plus the
    array-literal ELEMENT sites now go through `emit_init_value`, which
    descends `emit_expr_from_nir` when the lowered expr carries the exact AST
    node (from_ast is 1:1, guarded by `nir.node === value`) and falls back to
    `build_node` otherwise. Array literals lower to a NIR `call` whose args
    are the index-aligned elements, consumed via `nir_array_element` — the
    same shape the return path uses. The trailing-newline guard replicates
    build_node's with_semicolon tail (a `var f = func` decl adds none; under
    a NIR ctx that shape actually forces the whole-function fallback, so the
    guard is parity, not load-bearing).
  - `assign` → `build_assignment_node(node, status, nir_rhs)` — all 26 direct
    RHS emissions (plain/compound, mutable-param, ref-param, promoted-reg
    fast path, field stores, nullable-struct slots via the threaded
    `build_nullable_struct_assignment`) go through `emit_rhs_value`.
    `get_source_address` (address-position RHS) and `build_swap` (the swap
    expr, not the RHS) stay on the plain path for now — same bytes today.
  - `eval` → `emit_expr_from_nir(nstmt.expr, status)` + newline tail. One
    replicated side effect: build_node's with_semicolon path stamps a bare
    nursery-spawn statement (`is_statement`) fire-and-forget; the eval arm
    re-stamps it, or the spawn would emit a joined task (byte-identity test
    asserts this stays exact).
- The remaining delegated kinds (`break`/`continue`/`exit`/`raw`/`spawn`/
  `async_block`/`nested_func`/`anon_struct`/`opaque`) have no value positions;
  their takeover is about control flow, not the expression seam.

Proof: byte-IDENTICAL `.s` versus the emission cursor disabled —
`test/emit_nir.test.ts` extended to 25 tests (declare/assign/eval across
literal/op/cast/grouped/call/struct-ctor/fixed-array initializer shapes;
plain + compound + field + indexed + string-concat assignments; bare call +
method-call evals; a bare `nursery.spawn` statement whose fire-and-forget
stamp must survive the seam) AND the full `bench/nomen/*.nm` corpus.
Mutation checks (markers injected into each of the three new arms) confirmed
all three execute on the first test's body and were reverted. Behavioral
...
### C expression seam (DONE)

The C backend's remaining value positions now descend the canonical
expression seam — mirroring aarch64 tranches 3–5, and completing the C side
of the dispatch (`return`/`declare`/`assign`/`eval` were the last delegated
kinds with value positions).

- `src/build_c/emit_nir.ts` — `emit_stmt_from_nir` gained the four arms plus
  `emit_expr_from_nir` (the expression seam, exhaustive over NirExpr) and
  `nir_array_elements` (array literals lower to a `call` whose args are the
  index-aligned elements; consumed by the declaration and return
  array-literal sites). A new `with_semicolon_tail` helper replicates
  build_node's with_semicolon suffix byte-exactly — including the C
  backend's HISTORICAL extra `;` line after returns and the deferred
  string-release flush, both of which the delegated path emits and the arms
  must too.
  - C-side divergence from the aarch64 seam: the `wrap` arm routes through
    `build_node` UNCONDITIONALLY. C's `grouped` case emits literal
    parentheses into the expression text (aarch64's is a pure value
    pass-through), so the aarch64-style recursion into the NIR inner would
    drop parens — caught immediately by the knucleotide corpus diff
    (`((((c >> 1L)) & 3L))` vs `(((c >> 1L)) & 3L)`).
  - The `eval` arm re-stamps bare nursery spawns as fire-and-forget
    (`is_statement`), replicating build_node's with_semicolon side effect.
  - The `assign` arm passes `nstmt.node` (the INNER AssignmentNode, for
    let-wrapped arrow-arm assigns) — same as aarch64.
- `build_c/build_return_node.ts` — takes `nir_value`; value emissions
  (main `_return_val` initializer, the nested `return_assign` path, and the
  array-literal element initializer) descend the seam via a local
  `emit_return_value` (guarded by `nir.node === value`, falling back to
  `build_node`). Every ownership/borrow decision (strdup rules, join-slot
  routing, field-override application) stays on the AST node.
- `build_c/build_declaration_node.ts` — takes `nir_init`/`nir_swap`; local
  `emit_init_value`/`emit_swap_value` route the initializer sites (trait-
  class `void*` locals, nullable-struct init, heap-array-literal memcpy,
  strdup'd literals, type-erasure casts, trait pointer-array elements) and
  the decl-swap replacement through the seam.
- `build_c/build_assignment_node.ts` — takes `nir_rhs`/`nir_swap`; local
  `emit_rhs_value` routes ALL RHS value positions (trait vtable dispatch
  setter, string-field temp, trait-class and ref-class-param reassigns,
  string re-concat temp + literal strdup, nullable-struct slot, ref-string
  param, generic RHS) plus the swap replacement. Address positions stay on
  the plain path (the `{ rhs = swap; }` source re-emit is an lvalue; ref
  locals' `= &` re-pointing takes names only) — matching the aarch64
  precedent. View-string arg marshalling (`c_view_string_arg`) stays
  unthreaded, also matching aarch64 (`emit_view_string_arg`).
- `build_c/build_array_values_node.ts` — optional `nir_elements` param so
  array-literal elements descend the seam at both call sites.

Proof: byte-IDENTICAL C versus the emission cursor disabled —
`test/emit_c_nir.test.ts` extended to 24 tests (returns of every expression
shape incl. nested-flow arms, float/string returns; array-literal returns
riding the NIR element facts; declare/assign/eval across literal/op/grouped/
cast/string-concat/struct-ctor/fixed-array/nullable shapes; bare
nursery-spawn fire-and-forget; nullable-struct declares + null/non-null
assignments; variable-RHS and field-RHS assignment swaps; value-struct
declaration swap; behavioral run → `13 0.750000 hi there! 14 set 9 13 27
45`) AND the full `bench/nomen/*.nm` corpus. Mutation checks (markers
injected into all four new arms) confirmed each arm executes on the probe
programs and were reverted. Full suite green. Perf: byte-identical output by
design.

Side find (pre-existing, recorded in FOLLOWUP.md): `for v of <call>()`
crashes the C backend when the returned array has no compile-time length
