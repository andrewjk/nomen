...
`;
		...
		await check_output("ansi_red", result, "\x1b[31merr\x1b[0m");
	});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_green_blue", result, "\x1b[32mok\x1b[0m\x1b[34minfo\x1b[0m");
	});
});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_bg_red", result, "\x1b[41mERROR\x1b[0m");
	});
});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_bold", result, "\x1b[1mtitle\x1b[0m");
	});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_underline", result, "\x1b[4mlink\x1b[0m");
	});
});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_interpolate", result, "\x1b[41mERROR\x1b[0m: it didn't work");
	});
...
`;
		...
		expect(parsed.errors).toEqual([]);
		const result = build(parsed.root, { arch: "aarch64", platform: "macos", audit: true });
		await check_output("ansi_nested", result, "\x1b[1m\x1b[32msuccess\x1b[0m\x1b[0m");
	});
});
