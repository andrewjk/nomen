...
// Struct Array_Person
void *_Array_Person_Viewable_funcs[] = {};
void *_Array_Person_traits[] = {NULL, NULL, NULL, NULL, NULL, &_Array_Person_Viewable_funcs};
struct Array_Person Array_Person_init(long length)
{
struct Array_Person _self;
_self._vt = &_Array_Person_traits;
_self.length = length;
return _self;
}
struct Person Array_Person_at(struct Array_Person *self, long index)
{
struct Array_Person _self = *self;
struct Person* _data = (struct Person*)((char*)self + sizeof(*self));
return _data[index];
}
struct Person Array_Person_first(struct Array_Person *self)
{
struct Array_Person _self = *self;
struct Person* _data = (struct Person*)((char*)self + sizeof(*self));
return _data[0];
}
void Array_Person_set(struct Array_Person *self, long index, struct Person *value)
{
struct Person* _data = (struct Person*)((char*)self + sizeof(*self));
// 0 is 1 when struct Person is string: the slot must own a heap copy
...
// leak the previous (*value). 0 for all other struct Person (plain assignment).
#if 0
nomen_free_wrap(_data[index]);
_data[index] = (struct Person)nomen_strdup_wrap((*value));
#else
_data[index] = (*value);
#endif
}
struct Person Array_Person_at_end(struct Array_Person *self)
{
struct Array_Person _self = *self;
struct Person* _data = (struct Person*)((char*)self + sizeof(*self));
return _data[self->length - 1];
}
nomen_view Array_Person_slice(struct Array_Person *self, long start, long end)
{
struct Array_Person _self = *self;
nomen_view _r;
struct Person* _data = (struct Person*)((char*)self + sizeof(*self));
...
void* Array_Person_with(struct Person *value, long count)
{
long _header = 16;
void* _result = nomen_malloc_wrap(_header + count * 24);
((long*)_result)[1] = count;
struct Person* _data = (struct Person*)((char*)_result + _header);
for (long i = 0; i < count; i++) {
// 0 is 1 when struct Person is string (a `char*` pointer whose
// backing bytes must be heap-copied per slot — otherwise all n
// slots would share one pointer and auto_free would n-free it).
// For all other struct Person it is 0 and the plain assignment runs.
#if 0
_data[i] = (struct Person)nomen_strdup_wrap((*value));
#else
_data[i] = (*value);
#endif
}
...
void* Array_Person_add(struct Array_Person *self, struct Array_Person *other)
{
struct Array_Person _self = *self;
...
void* _result = nomen_malloc_wrap(_header + _result_len * 24);
((long*)_result)[0] = 0;
((long*)_result)[1] = _result_len;
struct Person* _data = (struct Person*)((char*)_result + _header);
struct Person* self_data = (struct Person*)((char*)self + _header);
struct Person* other_data = (struct Person*)((char*)other + _header);
memcpy(_data, self_data, _self_len * 24);
memcpy(_data + _self_len, other_data, _other_len * 24);
return _result;
}
void* Array_Person_mul(struct Array_Person *self, long other)
{
struct Array_Person _self = *self;
long _self_len = self->length;
...
memcpy(_data + i * _self_len, self_data, _self_len * 24);
}
return _result;
}

// Struct Person
struct Person Person_init(char* name, long age)
{
struct Person _self;
_self.name = name;
_self.age = age;
...
{
nomen_free_wrap(self->name);
}

// Func main
int main()
{
struct Array_Person* v;
v = nomen_malloc_wrap(sizeof(struct Array_Person) + 2L * sizeof(struct Person));
v->_vt = 0;
v->length = 2L;
memcpy((char *)v + sizeof(struct Array_Person), (struct Person[]){Person_init("Alice", 30L), Person_init("Bob", 25L)}, 2L * sizeof(struct Person));
;
char* out = nomen_strdup_wrap("");
{
long __for_idx_p;
for (__for_idx_p = 0L; __for_idx_p < v->length; __for_idx_p++)
{
struct Person p = Array_Person_at(v, __for_idx_p);
char* _reassign_1 = string_add(out, p.name);
nomen_free_wrap(out);
out = _reassign_1;
;
}
}
char* _param_0 = string_to_string(out);
