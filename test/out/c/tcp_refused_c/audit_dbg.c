...
void *nomen_malloc_wrap(unsigned long size) {
	...
	long c = __atomic_add_fetch(&nomen_malloc_count, 1, __ATOMIC_SEQ_CST);
	fprintf(stderr, "ALLOC %p size=%lu count=%ld\n", ptr, size, c);
	return ptr;
}
...
void nomen_free_wrap(void *ptr) {
	if (ptr) {
		free(ptr);
		long c = __atomic_sub_fetch(&nomen_malloc_count, 1, __ATOMIC_SEQ_CST);
		fprintf(stderr, "FREE  %p count=%ld\n", ptr, c);
	}
}
