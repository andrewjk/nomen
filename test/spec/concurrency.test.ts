...
`;
		const errors = compile_module(outside);
...
describe("spec: concurrency - user-defined async primitives", () => {
	test("an Awaitable class with the spawn-field contract gets the construction sugar", () => {
		// SPEC.md, "User-defined async primitives": the construction sugar
		// fires for any class conforming to Awaitable carrying the
		// task/result_slot/cancel_flag/future contract; launch is the
		// class's own methods, driving the packed-task machinery through
		// the Task.* library seam (no raw blocks — those are
		// System-library-only).
		const input = `
class Job : Awaitable {
...
`;
	...
	test("a missing contract field is a dedicated error", () => {
		const input = `
...
`;
		const errors = compile_module(input);
		expect(errors.some((e) => e.message.includes("spawn-field contract"))).toBe(true);
	});
});
