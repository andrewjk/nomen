...
test.skip("ziglings 108 labeled switch -- errors", () => {
...
test.skip("ziglings 108 labeled switch -- fixed", () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
});

test.skip("ziglings 108 labeled switch -- build", async () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
	await build_and_check_output(input, "108", "");
});
