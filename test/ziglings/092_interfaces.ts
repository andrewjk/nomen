...
test.skip("ziglings 092 interfaces -- errors", () => {
...
test.skip("ziglings 092 interfaces -- fixed", () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
});

test.skip("ziglings 092 interfaces -- build", async () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
	await build_and_check_output(input, "092", "");
});
