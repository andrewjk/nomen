...
test.skip("ziglings 096 memory allocation -- errors", () => {
...
test.skip("ziglings 096 memory allocation -- fixed", () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
});

test.skip("ziglings 096 memory allocation -- build", async () => {
	const input = zig_source;
	const parsed = parse_with_imports(input);
	expect(parsed.errors).toEqual([]);
	await build_and_check_output(input, "096", "");
});
