 ...
 * `string` expression is wrapped into a nomen_view carrying its strlen (an
 * implicit owned→view borrow; the caller keeps ownership of the string).
 * Statement-expression form evaluates the operand exactly once.
 */
...
export function c_materialize_view_string(arg: BaseNode, status: BuildStatus) {
	...
	build_node(arg, status);
	status.code += `; char* _r = malloc(_t.len + 1); memcpy(_r, _t.ptr, _t.len); _r[_t.len] = 0; _r; })`;
}
