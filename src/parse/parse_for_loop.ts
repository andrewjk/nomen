...
export default function parse_for_loop(status: ParseStatus) {
	...
	// A bare `for of arr` means the loop variable is missing: consume the
	// `of` raw so the usual `Expected of` error fires below instead of a
	// misleading reserved-word error.
	const value = peek_current(status) === "of" ? consume(status) : consume_name(status);
