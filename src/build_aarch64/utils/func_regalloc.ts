...
/**
 * Whole-function register allocation (ASM_PLAN phase 4, first tranche).
 *
 * Before a function body is built, scan its AST once and reserve callee-saved
 * registers for the hottest scalar LOCALS for the ENTIRE function: every
 * read/write of a promoted variable hits the register instead of an
 ...
 * - ESCAPES exclude a variable from promotion:
 ...
 *     declarations would share the register) or collides with a parameter.
...
export function plan_function_promotions(func: {
	...
	if (decl_counts.size === 0) return new Map();
